...
export function createRolldownConfig(jbrowseGlobals, options) {
  const includeUMD =
  ...
  const npmConfig =
    ...
    defineConfig({
      ...
      plugins: [writeIndex(packageName, distPath)],
      output: [
        {
  ...
  const configs = []
  for (const conf of [umdConfig, esmBundleConfig, npmConfig, cjsConfig]) {
    if (conf) {
      configs.push(conf)
    }
  }
