...
const stateModelFactory = (
  ...
  const { OAuthConfigSchema, OAuthInternetAccountModelFactory } =
    AuthPlugin.exports
  return (
    InternetAccount.named('ApolloInternetAccount')
      .props({
        ...
        addSocketListeners() {
          ...
          socket.on('USER_LOCATION', (message) => {
            ...
            if (channel === 'USER_LOCATION' && userToken !== token) {
              ...
              console.log(`Collab: ${JSON.stringify(collaborator)}`)
              session.addOrUpdateCollaborator(collaborator)
            }
          })
      ...
      .actions((self) => {
        async function postUserLocation(userLoc: UserLocation[]) {
          ...
          //   ...userLoc[0],
          //   start: String(userLoc[0].start),
          //   end: String(userLoc[0].end),
          // }
const dummy: UserLocation[] = [{assemblyId: 'aa', refSeq: 'refseq', start: 100, end: 200}, {assemblyId: 'aa1', refSeq: 'refseq1', start: 400, end: 500}]
console.log('dummy:', JSON.stringify(dummy));
const params = new URLSearchParams();
dummy.forEach((e) => params.append('start', e.start.toString()))
dummy.forEach((e) => params.append('end', e.end.toString()))
console.log('params:', params.toString());

          const userLocForParams = new URLSearchParams();
        //   Object.entries(userLoc).forEach(([key, value]) => {
        //     if (Array.isArray(value)) {
        //         value.forEach(value1 => userLocForParams.append(key, value1.toString()))
        //     } else {
        //       userLocForParams.append(key, value.toString())
        //     }
        // });
          // const userLocForParams = {
          //   ...userLoc,
          ...
          //   end: String('234'),
          // }
          // let userLocation = new URLSearchParams();
//           userLocation.append('arrayparams', userLocForParams);
//           userLocation.append('arrayparams', userLocForParams);
//           userLocation.append('arrayparams', 'val3');
// console.log(`userParams: ${JSON.stringify(userLocForParams.toString())}`)
      ...
      //     }
      //     console.log(JSON.stringify(userLocForParams))
